...
* (folder) shoppingMall_project
  ...
  5) Ajax 요청 ( by axios.get )으로 상품 데이터 추가 '데이터바인딩': 더보기 버튼을 누르면 추가한 상품이 보여지는 기능 구현
  6) 상위 Component의 state를 여러 하위 Component로 props 전송하기: 재고 상태 표시 + 주문하기 버튼을 누르면 재고가 하나 
